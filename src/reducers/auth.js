...
export const accessWeb = createAsyncThunk('user/access', async ({ atkP,accidP, rftP }, { rejectWithValue }) => {
  try {

    return (await accessApi.accessWeb({ atkP,accidP, rftP })).data.data;
  } catch (error) {
...
const authSlice = createSlice({
  ...
  extraReducers: {
    ...
    [accessWeb.fulfilled]: (state, action) => {
      const { user, accessToken, refreshToken } = action.payload;
      state.loading = false;
